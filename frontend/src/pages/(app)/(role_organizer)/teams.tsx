...
import { TeamInfo } from "~/lib/models";
...
const Teams: Component<{}> = () => {
    ...
    const [allTeamInfo, setAllTeamInfo] = createSignal<TeamInfo[]>([]); 
    ...
    async function sendAlert(idNum: number) {
        ...
        if(res.status === 304) {
            toast.warning(
                "Már küldtek értesítést ennek a csapatnak"
            )
        }
        else {
            toast.success(
    ...
    return(
        ...
        <Table>
            <TableCaption>Csapatok</TableCaption>
            <TableHeader>
                <TableRow>
                            ...
                            <Dialog>
                                ...
                                <DialogContent>
                                    ...
                                    <div>
                                        ...
                                        <p class="ml-20">Neve: {team.supplementary_members[0].name!==""?team.supplementary_members[0].name:"Nincs"}</p>
                                        <p class="ml-20"> Osztálya: {team.supplementary_members[0].name!==""?team.supplementary_members[0].grade:"Nincs"}</p>
                                        <p>intézménye: {team.school}</p>
                                        <p>Felkészítő tanár: {team.teacher_name}</p>
                                        <p>Kategória: {team.category}</p>
                                        <p>Program nyelve: {team.prog_lang}</p>


                                    </div>
                                    <DialogFooter>
                                        <Button variant="destructive" onClick={()=>{sendAlert(team.id);}}>Figyelmeztető küldése</Button>
                                    </DialogFooter>
                                </DialogContent>
                            </Dialog>
                        </TableCell>
                        <TableCell>
                            <Button class="bg-green-600 hover:bg-green-500" onClick={()=>{changeaStatus(team.id)}}><FaSolidCheck/></Button>
                        </TableCell>
                        <TableCell>
                        <Dialog>
                            <DialogTrigger>
